...
interface CurrentState {
  dsn_etablissement: DSNEtablissement | null
  dsn_individu: DSNIndividu | null
  dsn_contrat: DSNContrat | null,
    ...
    if (isBlockStart(code, value)) {
      ...
      switch (getBlockPrefix(code)) {
        ...
        case DSN_BLOCK_CODES.ETABLISSEMENT:
          if (current_state.dsn_etablissement) {
            mapEtablissementField(current_state.dsn_etablissement.etablissement, fieldNum, value)
          }
          break;
        case DSN_BLOCK_CODES.CONTRAT:
          if (current_state.dsn_contrat) {
            mapContratField(current_state.dsn_contrat.contrat, fieldNum, value)
          }
          break;

        case DSN_BLOCK_CODES.INDIVIDU:
          if (current_state.dsn_individu) {
            mapIndividuField(current_state.dsn_individu.individu, fieldNum, value)
          }
          break;
...
function parseBloc(dsn_document: DSNDocument, code: string, current_state: CurrentState) {
  switch (code) {
    case DSN_BLOCK_CODES.ETABLISSEMENT:
      current_state.dsn_etablissement = {
        etablissement: {},
        dsn_individu: []
      }
      dsn_document.dsn_etablissement.push(current_state.dsn_etablissement)
      current_state.dsn_individu = null
      current_state.dsn_contrat = null
      current_state.versement = null
      current_state.remuneration = null
      break;
    case DSN_BLOCK_CODES.INDIVIDU:
      current_state.dsn_individu = {
        individu: {},
        dsn_contrat: [],
      }
      current_state.dsn_etablissement?.dsn_individu.push(current_state.dsn_individu)
      current_state.dsn_contrat = null
      current_state.versement = null
      current_state.remuneration = null
      break;
    case DSN_BLOCK_CODES.CONTRAT:
      current_state.dsn_contrat = {
        contrat: {},
        versements: [],
        remunerations: [],
      }
      current_state.dsn_individu?.dsn_contrat.push(current_state.dsn_contrat)
      current_state.versement = null
      current_state.remuneration = null
      break;
    case DSN_BLOCK_CODES.VERSEMENT:
      current_state.versement = {}
      current_state.dsn_contrat?.versements.push(current_state.versement)
      break;
    case DSN_BLOCK_CODES.REMUNERATION:
      current_state.remuneration = {}
      current_state.dsn_contrat?.remunerations.push(current_state.remuneration)
      break;
    default:
      break;
  }
...
export function mapDeclarationField(declaration: Declaration, field_num: string, value: string): void {
  switch (field_num) {
...
export function mapContactField(contact: ContactEmetteur, field_num: string, value: string): void {
  switch (field_num) {
...
export function mapEntrepriseField(entreprise: Entreprise, field_num: string, value: string): void {
  switch (field_num) {
...
export function mapEnvoiField(envoi: Envoi, field_num: string, value: string) {
  switch (field_num) {
...
function mapIndividuField(individu: Individu, field_num: string, value: string): void {
  switch (field_num) {
...
function mapContratField(contrat: Contrat, field_num: string, value: string): void {
  switch (field_num) {
...
function parseLine(line: string): DSNLine | null {
  ...
  // Match pattern: CODE,'VALUE' where VALUE can be empty
